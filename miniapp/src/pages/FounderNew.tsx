...
const PRICING_MODELS = ["Free - no charge", "Subscription", "One-time", "Freemium", "Usage-based", "Other"];
...
type StepDef = { key: string; title: string; render: () => JSX.Element; valid: () => boolean; final?: boolean; };
...
export default function FounderNew() {
  ...
  const steps: StepDef[] = [
    ...
    },
    {
      ...
      render: () => (
        <input placeholder="“We do [this] so users can unlock [value]”" value={pitch} onChange={e=>setPitch(e.target.value)} />
      ),
      valid: () => true
    },
    {
    ...
    },
    {
      ...
      render: () => (
        <>
          {isFreeChoice && <p className="form-hint">Price points are disabled because you selected a free / no-charge model.</p>}
          <div className="stack">
            <div className="price-input"><input placeholder="Price Point 1" value={price1} onChange={e=>setPrice1(e.target.value)} disabled={isFreeChoice} /></div>
            <div className="price-input"><input placeholder="Price Point 2 (optional)" value={price2} onChange={e=>setPrice2(e.target.value)} disabled={isFreeChoice} /></div>
            <div className="price-input"><input placeholder="Price Point 3 (optional)" value={price3} onChange={e=>setPrice3(e.target.value)} disabled={isFreeChoice} /></div>
          </div>
        </>
      ),
      valid: () => isFreeChoice || numberize(price1) !== null || numberize(price2) !== null || numberize(price3) !== null
    },
    {
      key: "actions",
            ...
            {ACTION_OPTIONS.map(o=>(
              <div className="v-choice" key={o.key}>
                <input id={`act-${o.key}`} type="checkbox"
                       ...
                       onChange={()=> setSelectedActions(a=> a.includes(o.key)? a.filter(x=>x!==o.key) : [...a,o.key])} />
    ...
    },
    {
      ...
      render: () => (
        <textarea className="min-h-[100px]" value={founderFeedback} onChange={e=>setFounderFeedback(e.target.value)} placeholder="What should we add or change?" />
      ),
      valid: () => true,
      final: true
    }
  ...
  const next = () => { setErr(null); if (!cur.valid()) { setErr("Please complete this step."); clearErrSoon(); return; } setStep(s => Math.min(s+1, steps.length-1)); };
  ...
  async function createSession() {
    ...
    const emailNorm = normEmail(founderEmail);
    ...
    const payload: FounderInputsStreamlit = {
      ...
      target_audience: null, target_action: null, follow_up_action: null,
    };

    try {
      const r = await fetch(`${API}/session_sb`, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(payload) });
      const j = await r.json();
      if (!r.ok) { setErr(j?.detail || "Failed to create session"); clearErrSoon(); return; }
      if (j?.session_id) {
        localStorage.setItem("verityFounderEmail", emailNorm);
        const url = `/founder/share?sid=${encodeURIComponent(j.session_id)}`;
        nav(url, { state: { sid: j.session_id, share: j.share_link || null } });
      }
    } catch (e: any) {
      ...
      <div className="card email-strip">
        <div className="row" style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:12, alignItems:"end" }}>
          <div>
            ...
            <input placeholder="you@example.com" value={founderEmail} onChange={(e)=>setFounderEmail(e.target.value)} />
          </div>
          <button className="btn_primary" onClick={()=>{
            if (!emailValid) { setErr("Enter a valid email"); clearErrSoon(); return; }
            localStorage.setItem("verityFounderEmail", normEmail(founderEmail));
          }}>
            Use this email
          </button>
        </div>
