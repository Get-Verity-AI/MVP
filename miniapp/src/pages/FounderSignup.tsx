...
export default function FounderSignup() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Then call your existing backend API
      ...
      if (!j?.ok) {
        throw new Error("Failed to register founder in backend");
      }
  ...
  return (
    <div className="container max-w-2xl mx-auto p-6">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          <label className="text-sm opacity-80">Name/Company Name (optional)</label>
          <input 
            className="w-full px-3 py-2 rounded-xl border" 
            value={name} 
      ...
      <p className="mt-4 text-sm opacity-70">
        Already have an account? <Link to="/founder/signin" className="text-blue-600">Sign in</Link>
      </p>
    </div>
  );
}
